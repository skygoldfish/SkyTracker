...
class C01(XAReal):
    ...
    def OnReceiveRealData(self, szTrCode):
        ...
        result['최근월체결가격'] = float(self.ActiveX.GetFieldData(self.OUTBLOCK, "spdprc1"))
        result['차근월체결가격'] = float(self.ActiveX.GetFieldData(self.OUTBLOCK, "spdprc2"))
        result['매도수구분'] = self.ActiveX.GetFieldData(self.OUTBLOCK, "dosugb")
        result['계좌번호1'] = self.ActiveX.GetFieldData(self.OUTBLOCK, "accno1")
        result['시장조성호가구분'] = self.ActiveX.GetFieldData(self.OUTBLOCK, "sihogagb")
...
class OC0(XAReal):
    ...
    def AdviseRealData(self, 종목코드):
        ...
        if 종목코드 not in list(self.onadvise.keys()):
            self.onadvise[종목코드] = ''
            self.ActiveX.SetFieldData(self.INBLOCK, "optcode", 종목코드)
            self.ActiveX.AdviseRealData()
...
class OVH(XAReal):
    ...
    def OnReceiveRealData(self, szTrCode):
        ...
        매수호가건수 = []
        for i in range(1,6):
            매도호가.append(float(self.ActiveX.GetFieldData(self.OUTBLOCK, "offerho%s" % i).strip()))
            매수호가.append(float(self.ActiveX.GetFieldData(self.OUTBLOCK, "bidho%s" % i).strip()))
            매도호가잔량.append(int(self.ActiveX.GetFieldData(self.OUTBLOCK, "offerrem%s" % i).strip()))
            매수호가잔량.append(int(self.ActiveX.GetFieldData(self.OUTBLOCK, "bidrem%s" % i).strip()))
            매도호가건수.append(int(self.ActiveX.GetFieldData(self.OUTBLOCK, "offerno%s" % i).strip()))
            매수호가건수.append(int(self.ActiveX.GetFieldData(self.OUTBLOCK, "bidno%s" % i).strip()))
        ...
        result['매도호가총건수'] = int(self.ActiveX.GetFieldData(self.OUTBLOCK, "totoffercnt").strip())
        result['매수호가총건수'] = int(self.ActiveX.GetFieldData(self.OUTBLOCK, "totbidcnt").strip())
        result['매도호가총수량'] = int(self.ActiveX.GetFieldData(self.OUTBLOCK, "totofferrem").strip())
        result['매수호가총수량'] = int(self.ActiveX.GetFieldData(self.OUTBLOCK, "totbidrem").strip())
